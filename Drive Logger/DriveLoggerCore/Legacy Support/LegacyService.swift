...
import WidgetKit
import UIKit
public struct LegacyDrive: Codable, Identifiable {
...
public struct DriveLoggerAppState: Codable {
    ...
    public var drives: [LegacyDrive] = []
    public var drivesSortedByDate: [LegacyDrive] = []
...
public class DriveLoggerService {
   ...
   public func hapticResponse() {
    ...
    public func retreiveCurrentDrive() -> LegacyCurrentDrive? {
        var attemptedRetreival: LegacyCurrentDrive? = nil
        do {
            attemptedRetreival = try Disk.retrieve("currentDrive.json", from: .sharedContainer(appGroupName: "group.com.ronanfuruta.drivelogger"), as: LegacyCurrentDrive.self, decoder: decoder)
        } catch {
    ...
    public func saveCurrentDrive(_ currentDrive: LegacyCurrentDrive?) {
    ...
    public func isNightDrive(_ drive: LegacyDrive) -> Bool {
        let sunsetHour = 19
        let sunriseHour = 5
    ...
    public func updateDrive(_ drive: LegacyDrive, state: DriveLoggerAppState) -> DriveLoggerAppState{
        let id = drive.id
    ...
    public func deleteDrive(_ drive: LegacyDrive, state: DriveLoggerAppState) -> DriveLoggerAppState{
        let id = drive.id
        var drives = state.drives
        let index = drives.firstIndex(where: {drive in drive.id == id})
        if let index = index {
